...
public abstract class Entity {
    ...
    public void setCollisionBox(float offsetX, float offsetY, float width, float height) {
        bboxOriginX = offsetX;
        bboxOriginY = offsetY;
        bboxWidth = width;
        bboxHeight = height;
    ...
    public void setCollisionBox(Slot slot, SkeletonBounds skeletonBounds) {
        ...
        setCollisionBox(minX - x, minY - y, maxX - minX, maxY - minY);
    }
    ...
    public boolean containsPoint(float x, float y) {
        return x >= getBboxLeft() && x <= getBboxRight() && y >= getBboxBottom() && y <= getBboxTop();
    }
}